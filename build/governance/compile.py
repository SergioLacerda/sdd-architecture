...
class GovernanceCompiler:
    ...
    def calculate_fingerprint(self, data: Dict[str, Any]) -> str:
        ...
        data_copy = {k: v for k, v in data.items() if k not in ["fingerprint", "fingerprintpackages_salt"]}
    ...
    def generate_governance_files(self) -> None:
        ...
        fingerprintpackages = self.calculate_fingerprint(governancepackages)
        governancepackages["fingerprint"] = fingerprintpackages

        # Client fingerprint uses core fingerprint as salt
        combined_for_client = {**governance_client, "fingerprintpackages_salt": fingerprintpackages}
        fingerprint_client = self.calculate_fingerprint(combined_for_client)
        governance_client["fingerprint"] = fingerprint_client
        governance_client["fingerprintpackages_salt"] = fingerprintpackages
        ...
        core_path.write_text(json.dumps(governancepackages, indent=2, ensure_ascii=False))
        print(f"   ✓ {core_path.relative_to(self.core_path.parent)}")
        ...
        print(f"   ✓ {client_path.relative_to(self.core_path.parent)}")
        ...
        metadatapackages = {
            ...
            "type": "GOVERNANCEpackages",
            "readonly": True,
            "compiled_at": governancepackages["compiled_at"],
            "fingerprint": fingerprintpackages,
            "items_count": len(self.core_items),
        }

        metadatapackages_path = output_dir / "metadata-core.json"
        metadatapackages_path.write_text(json.dumps(metadatapackages, indent=2))
        print(f"   ✓ {metadatapackages_path.relative_to(self.core_path.parent)}")
        ...
        metadata_client = {
            ...
            "fingerprintpackages_salt": fingerprintpackages,
        ...
        print(f"   ✓ {metadata_client_path.relative_to(self.core_path.parent)}")
        ...
        print(f"  SHA256(governance-core.json) = {fingerprintpackages}")
        print()
        print("Client fingerprint (DERIVED from core):")
        print(f"  SHA256(fingerprintpackages + governance-client.json) = {fingerprint_client}")
        print()
        print(f"Client includes: fingerprintpackages_salt = {fingerprintpackages}")
        print()
