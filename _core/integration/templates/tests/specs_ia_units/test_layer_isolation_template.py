...
COPY THIS FILE TO: tests/specs_ia_units/test_layer_isolation.py
...
class TestLayerIsolation:
    ...
    def test_domain_never_imports_infrastructure(self):
        ...
        for file in domain_files:
            ...
            assert (
                "from src.[project_name].infrastructure" not in content
            ), f"{file}: domain imports infrastructure (Rule 1 violation)"
            assert (
                "import src.[project_name].infrastructure" not in content
            ), f"{file}: domain imports infrastructure (Rule 1 violation)"
    ...
    def test_domain_never_imports_frameworks(self):
        ...
        for file in domain_files:
            ...
            for framework in forbidden_frameworks:
                assert framework not in content, f"{file}: domain imports framework {framework.split()[-1]} (Rule 1 violation)"
    ...
    def test_application_uses_ports_not_infrastructure(self):
        ...
        for file in app_files:
            ...
            if "storage" in content.lower() and "Storage" in content:
                ...
                assert not (
                    has_direct_import and not has_port_reference
