...
Examples:
  ...
  Test phases 1-2:
    ...
    parser.add_argument(
        "--language", choices=["java", "python", "js"], help="Target programming language (interactive if not specified)"
    )

    # Optional: Mandate selection
    parser.add_argument("--mandates", help="Comma-separated mandate IDs (e.g., M001,M002)")

    # Optional: Output directory
    parser.add_argument("--output", type=Path, help="Output directory for generated project")
    ...
    parser.add_argument("--test-phases", help="Test specific phases (e.g., 1-2, 1-7, just 1)")

    # Optional: Interactive mode
    parser.add_argument(
        "--interactive", action="store_true", help="Run in interactive guided mode (default if no arguments provided)"
    )
...
def main():
    ...
    should_run_interactive = args.interactive or (
        not args.language and not args.mandates and not args.output and not args.test_phases
    )
    ...
    if args.test_phases:
        ...
        orchestrator.run_full_pipeline(language=args.language or "python", mandates=mandates, output_dir=args.output)
        return 0
    ...
    success = orchestrator.run_full_pipeline(language=args.language or "python", mandates=mandates, output_dir=args.output)
