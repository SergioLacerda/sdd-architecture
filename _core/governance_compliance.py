...
class GovernanceComplianceValidator:
    ...
    def validate_all(self) -> Tuple[bool, Dict]:
        ...
        if self.governance_path.exists():
            ...
            results['violations'].append("Policy 1: governance-core.json not found at .sdd/governance-core.json")
    ...
    def get_mandatory_fix_steps(self, results: Dict) -> List[str]:
        ...
        if "Policy 3" in " ".join(results['violations']):
            ...
            fixes.append(f"  Edit .sdd/governance-core.json and set:")
            fixes.append(f'    "seedlings": {{ "active": ["my-domain"] }}')
            step += 1
        ...
        if "Policy 5" in " ".join(results['violations']):
            ...
            fixes.append(f"  Edit .sdd/governance-core.json and set:")
            fixes.append(f'    "enforcement": "standard"')
            step += 1
        ...
        if "Policy 6" in " ".join(results['violations']):
            ...
            fixes.append(f"  Edit .sdd/governance-core.json and set:")
            fixes.append(f'    "phases": {{ "completed": [0] }}')
            step += 1
        
        fixes.append(f"\nStep {step}: Verify compliance")
        fixes.append(f"  python3 _core/governance_compliance.py --verify")
