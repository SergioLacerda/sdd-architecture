...
class AgentHandshakeProtocol:
    ...
    def should_run_handshake(self, user_input: str) -> bool:
        ...
        technical_keywords = [
            # Direct queries
            "estou conectado", "connected", "status", "health",
            "handshake", "verificar", "validate",
            # Project references
            "código", "code", "arquivo", "file", "implementar",
            "implement", "arquitetura", "architecture",
            "estrutura", "structure", "projeto", "project",
            # System references
            ".sdd", "governance", "mandates", "guidelines",
            "spec", "fase", "phase", "wizard",
            # Config references
            ".vscode", ".cursor", ".ia", ".github"
        ]
        
        # Keywords that suppress handshake (casual conversation)
        # Use longer/more specific phrases to avoid false positives
        casual_keywords = [
            "oi", "olá", "hello world", "hi there", "obrigado", "thanks for",
            "muito legal", "pretty cool", "que legal", "muito nice", "como você está",
            "how are you", "qual seu nome", "your name", "qual é seu nome",
            "piada", "joke", "meme", "funny"
        ]
    ...
    def _layer_1_discovery(self) -> Tuple[str, List[ValidationResult]]:
        ...
        results.append(ValidationResult(
            name=".sdd/ directory",
            passed=sdd_exists or True,  # Optional
            message=f"Found" if sdd_exists else "Not initialized (optional)",
            layer="DISCOVERY"
        ))
        
        # Check governance-core.json
        governance_path = self.project_root / ".sdd-compiled" / "governance-core.json"
        governance_exists = governance_path.exists()
        results.append(ValidationResult(
            name="governance files",
            passed=governance_exists or True,  # Optional
            message=f"Found" if governance_exists else "Not compiled (optional)",
            layer="DISCOVERY"
        ))
    ...
    def _layer_4_governance_health(self) -> Tuple[str, List[ValidationResult]]:
        ...
        results.append(ValidationResult(
            ...
            message=f"Present ({critical_present}/{len(critical_files)})" if critical_ok else f"Missing ({critical_present}/{len(critical_files)})",
            layer="GOVERNANCE_HEALTH"
        ))
