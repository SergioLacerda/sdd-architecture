...
def test_core_governance_immutability(target_level="strict"):
    ...
    wizard_script = Path("_core/.sdd-wizard/src/interactive_mode.py")
    subprocess.run([
        sys.executable, str(wizard_script),
        "--headless",
        "--output", str(test_dir),
        "--lang", "python",
        "--enforcement", target_level,
        "--all-mandates"
    ], check=True)
    ...
    assert any("Integrity Failure" in v for v in results_after_attack['violations']), \
