...
class MigrationValidator:
    ...
    def validate_mandate_spec(self, spec_file: Path, expected_count: int = 15) -> bool:
        ...
        if not spec_file.exists():
            self.results.append(
                ValidationResult(
                    check_name="mandate_spec_exists", passed=False, message=f"mandate.spec not found at {spec_file}"
                )
            )
            return False
        ...
        if not ids_valid:
            ...
            self.results.append(
                ValidationResult(
                    check_name="sequential_ids", passed=True, message="All IDs properly sequenced", details={"ids": matches}
                )
            )
    ...
    def validate_guidelines_dsl(self, dsl_file: Path) -> bool:
        ...
        if not dsl_file.exists():
            self.results.append(
                ValidationResult(
                    check_name="guidelines_dsl_exists", passed=False, message=f"guidelines.dsl not found at {dsl_file}"
                )
            )
            return False
    ...
    def get_report(self) -> Dict[str, Any]:
        ...
        return {
            "summary": {"passed": passed, "total": total, "success_rate": f"{(passed/total*100):.1f}%" if total > 0 else "0%"},
