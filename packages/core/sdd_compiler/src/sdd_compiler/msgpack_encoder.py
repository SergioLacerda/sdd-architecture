...
class MessagePackEncoder:
    ...
    def encode(self, compiled_data: Dict[str, Any]) -> bytes:
        ...
        self.metrics.string_pool_size = len(json.dumps(compiled_data.get("string_pool", [])).encode("utf-8"))
        ...
        for m in compiled.get("mandates", []):
            ...
            for key, value in m.items():
                if isinstance(value, int) and value < len(string_pool):
                    ...
                    mandate[key] = [string_pool[v] if isinstance(v, int) and v < len(string_pool) else v for v in value]
                else:
                    mandate[key] = value
        ...
        for g in compiled.get("guidelines", []):
            ...
            for key, value in g.items():
                if isinstance(value, int) and value < len(string_pool):
                    ...
                    guideline[key] = [string_pool[v] if isinstance(v, int) and v < len(string_pool) else v for v in value]
                else:
                    guideline[key] = value
