...
""")
        ...
        config = {"language": language, "adoption_level": adoption_level, "generated_at": datetime.now().isoformat()}
    ...
    def phase_1_generate_templates(self) -> bool:
        ...
        try:
            ...
            # Adjust repo_root if we're inside packages (wizard.sh changes to packages)
            repo_root = self.repo_root
            if repo_root.name == "packages":
                repo_root = repo_root.parent

            core_path = repo_root / "packages"
            output_path = repo_root / "sdd-generated" / "phase-1-choices"

            generator = Phase1Generator(core_path, output_path, verbose=True, config=config)
...
""")
    ...
    def phase_2_show_instructions(self) -> bool:
        ...
        # Adjust repo_root if we're inside packages
        repo_root = self.repo_root
        if repo_root.name == "packages":
            repo_root = repo_root.parent
...
""")
    ...
    def phase_4_generate_project(self) -> bool:
        ...
        try:
            ...
            # Load config from sdd_wizard-config.json
