...
Phase 1: Generate markdown templates with status fields
  Input: mandate.spec, guidelines.dsl from packages/core/
...
class Phase1Generator:
    ...
    def __init__(self, core_path: Path, output_path: Path, verbose: bool = False, config: dict = None):
    ...
    def parse_mandate_spec(self) -> bool:
        ...
        mandate_file = self.core_path / "core" / "mandate.spec"
        ...
        for match in re.finditer(pattern, content, re.MULTILINE | re.DOTALL):
            ...
            mandate = Mandate(id=mandate_id, type=type_, title=title, description=desc, category=category, rationale=rationale)
    ...
    def parse_guidelines_dsl(self) -> bool:
        ...
        guidelines_file = self.core_path / "core" / "guidelines.dsl"
        ...
        for match in re.finditer(pattern, content, re.MULTILINE | re.DOTALL):
            ...
            guideline = Guideline(id=guideline_id, type=type_, title=title, description=desc, category=category)
...
class Phase3Compiler:
    ...
    def create_structure(self) -> bool:
        ...
        try:
            ...
            self.log(f"Created .sdd structure: {dir}")
            return True
        except Exception as e:
    ...
    def copy_seedlings(self) -> bool:
        """Copy seedling templates from integration/templates to seedling/ directory"""
        try:
            # Look for seedling templates in integration
            source_seedling_dir = self.repo_root / "packages" / "integration" / "templates"
    ...
    def compile_with_pipeline_builder(self, items: Dict[str, List[Dict]]) -> bool:
        ...
        try:
            ...
            builder = PipelineBuilder(str(spec_path))
            ...
            # Create directories
            source = self.output_path / ".sdd" / "source"
            ...
            with open(core_file, "w", encoding="utf-8") as f:
                json.dump(result["governancepackages"], f, indent=2)
            ...
            print("❌ PipelineBuilder not available as package")
            print("   → Did you install sdd-build?")
            print(f"   Error: {e}")
            return False
...
"""
        ...
        try:
            ...
            builder_path = self.repo_root / "packages" / "build_scripts"
