...
Load compiled msgpack artifacts from sdd_compiler/runtime/
...
class GovernanceRuntimeLoader:
    ...
    def __init__(self, compiled_dir: str = None):
        ...
        Args:
            compiled_dir: Path to compiler/runtime directory.
                         If None, discovers from repository root.
        """
        if compiled_dir is None:
            # Discover from repository root: go up from this file
            # File is at: packages/interfaces/sdd_wizard/src/sdd_wizard/sdd_runtime/governance_runtime_loader.py
            # Need to go up 7 levels to reach repo root
            current = Path(__file__)
            for _ in range(7):
                current = current.parent
            repo_root = current
            compiled_dir = repo_root / "runtime" / "compiled"
        else:
            compiled_dir = Path(compiled_dir)

        self.compiled_dir = compiled_dir
        self.packages_data = None
        self._client_data = None
        self.packages_metadata = None
    ...
    def load_all(self) -> Dict[str, Any]:
        ...
        return {
            "status": "loaded",
            "core_items": len(self.packages_data.get("items", [])),
            "client_items": len(self._client_data.get("items", [])),
            "core_fingerprint": self.packages_metadata.get("fingerprint"),
            "client_fingerprint": self._client_metadata.get("fingerprint"),
        }

    def loadpackages(self) -> Dict[str, Any]:
    ...
    def load_client(self) -> Dict[str, Any]:
        ...
        msgpack_file = self.compiled_dir / "governance-client-template.compiled.msgpack"
    ...
    def getpackages_governance(self) -> Dict[str, Any]:
    ...
    def get_all_items(self) -> List[Dict[str, Any]]:
        ...
        core_items = self.getpackages_governance().get("items", [])
    ...
    def get_items_by_type(self, item_type: str) -> List[Dict[str, Any]]:
        ...
        return [item for item in items if item.get("type", "").upper() == item_type_upper]
    ...
    def get_salt_fingerprint(self) -> str:
        ...
        if self._client_metadata is None:
            self._load_metadata_only()
        return self._client_metadata.get("fingerprintpackages_salt")
    ...
    def _validate_integrity(self) -> bool:
        ...
        checks = [
            ("Core fingerprint", self._validatepackages_fingerprint()),
            ("Client fingerprint", self._validate_client_fingerprint()),
            ("Salt strategy", self._validate_salt_strategy()),
            ("Core readonly", self._validatepackages_readonly()),
            ("Client customizable", self._validate_client_customizable()),
            ("Core > 0 items", len(self.packages_data.get("items", [])) > 0),
            ("Client > 0 items", len(self._client_data.get("items", [])) > 0),
            ("Fingerprints different", self.getpackages_fingerprint() != self.get_client_fingerprint()),
        ]
    ...
    def _validatepackages_fingerprint(self) -> bool:
        """Validate core fingerprint is embedded"""
        core = self.getpackages_governance()
    ...
    def _validate_salt_strategy(self) -> bool:
        ...
        core_fp = self.getpackages_fingerprint()
        salt_fp = self.get_salt_fingerprint()
        return core_fp == salt_fp

    def _validatepackages_readonly(self) -> bool:
        """Validate core metadata has readonly flag"""
        return self.packages_metadata.get("readonly") is True
    ...
    def get_governance_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "core_fingerprint": self.getpackages_fingerprint(),
            "client_fingerprint": self.get_client_fingerprint(),
            "salt_fingerprint": self.get_salt_fingerprint(),
            "core_readonly": self.packages_metadata.get("readonly"),
            "client_customizable": self._client_metadata.get("customizable"),
        }

    def export_governance_json(self, filepath: str, includepackages: bool = True, include_client: bool = True) -> None:
        """Export governance as JSON for inspection/customization"""
        output = {}

        if includepackages:
            output["core"] = self.getpackages_governance()
