...
python3 _core/agent_handshake.py
...
python3 _core/health_check.py --verbose
```
Look for "git/.git" or "_core/" failures.
...
**Verification**:
```bash
python3 _core/health_check.py | grep "🟢"
...
ls -la _core/.sdd-wizard/templates/

# Fix: Restore from Phase 2
git checkout origin/main -- _core/.sdd-wizard/
...
| Policy | Failure | Fix |
|--------|---------|-----|
| governance-core.json exists | File missing | `mkdir -p _core/.sdd` |
| Valid JSON format | Invalid JSON | `python3 -m json.tool _core/.sdd/governance-core.json` |
...
| Health check passes | Failures | `python3 _core/health_check.py --force-recheck` |
...
# 4. Verify resolution
python3 _core/governance_compliance.py --verify --enforce strict
...
# Edit _core/.sdd/governance-core.json:
...
| Layer 2 | Health check failed | `python3 _core/health_check.py --force-recheck` |
| Layer 3 | Governance violated | `python3 _core/governance_compliance.py --fix-steps` |
...
# If Layer 2: python3 _core/health_check.py --force-recheck
# If Layer 3: python3 _core/governance_compliance.py --verify
# If Layer 4: python3 _core/governance_compliance.py --enforcement-check
...
python3 _core/agent_handshake.py --force-recheck
...
```bash
python3 _core/quiz_executor.py
# ❌ Quiz initialization failed
```

**Diagnosis**:
```bash
python3 _core/quiz_executor.py --verbose
```
...
| Issue | Fix |
|-------|-----|
| quiz_questions.json missing | Restore from GitHub: `git checkout origin/main -- _core/quiz_questions.json` |
| Invalid JSON format | `python3 -m json.tool _core/quiz_questions.json` |
| Health check failed | `python3 _core/health_check.py --force-recheck` |
| Wrong Python version | Ensure Python 3.8+ |

**Fix Steps**:
```bash
# 1. Check JSON validity
python3 -m json.tool _core/quiz_questions.json

# 2. Restore if needed
git checkout origin/main -- _core/quiz_questions.json
...
```bash
python3 _core/quiz_executor.py --topic governance
...
```bash
time python3 _core/health_check.py
...
```bash
time python3 _core/agent_handshake.py
...
│  ├─ Governance issue?
│  │  └─ Run: python3 _core/governance_compliance.py --verify
│  ├─ Config missing?
│  │  └─ Run: mkdir -p _core/.sdd
│  └─ Python version?
...
│  ├─ Layer 3 (governance)?
│  │  └─ Verify: python3 _core/governance_compliance.py --verify
│  └─ Layer 4 (enforcement)?
│     └─ Check: python3 _core/governance_compliance.py --enforcement-check
│
├─ Can't commit/push?
...
### Error: "ModuleNotFoundError: No module named '_core'"
...
```bash
# Validate JSON
python3 -m json.tool _core/.sdd/governance-core.json
...
```bash
# Check permissions
ls -la _core/health_check.py

# Make executable
chmod +x _core/health_check.py
chmod +x _core/agent_handshake.py
chmod +x _core/governance_compliance.py
...
0 */6 * * * cd /path/to/project && python3 _core/health_check.py >> logs/health.log 2>&1
```
...
# Hooks will auto-run:
# - Before commit: python3 _core/health_check.py
# - Before push: python3 _core/health_check.py --force-recheck
```
...
0 0 * * 0 cd /path/to/project && python3 _core/governance_compliance.py --verify
```
...
0 1 * * 0 rm -f _core/.sdd/agent_state.json
```
...
1. **Gather diagnostics**:
   ```bash
   python3 _core/health_check.py --verbose > diagnostics.log
   python3 _core/governance_compliance.py --verify >> diagnostics.log
...
2. **Check logs**:
   ...
   cat _core/.sdd/debug.log (if exists)
   ```
...
**A**: Open an issue with verbose output. Checks are adjustable in `_core/health_check.py`.
...
- [Governance Implementation](../_core/.sdd-wizard/templates/governance/base-seedling/GOVERNANCE_IMPLEMENTATION.md) - Set up governance
