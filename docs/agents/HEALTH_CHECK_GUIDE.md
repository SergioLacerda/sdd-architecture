...
[STRUCTURE]
✓ Core directories exist        (packages, docs, etc.)
✓ Python files present          (health_check.py, etc.)
✗ Config directory missing      (packages/.sdd/ missing)
...
4. CI/CD verification
   $ python3 packages/health_check.py || exit 1

5. After governance updates
   $ python3 packages/health_check.py --force-recheck --verbose
```
...
Required: packages, docs, tests, EXECUTION, INTEGRATION
✓ Pass: All directories present
✗ Fail: Missing packages or docs

# Fix: mkdir -p packages docs tests
...
Required: packages/.sdd/
✓ Pass: Directory exists
✗ Fail: Missing packages/.sdd/

# Fix: mkdir -p packages/.sdd/
...
# Fix: python3 packages/governance_compliance.py --verify
```
...
python3 packages/health_check.py --category git
python3 packages/health_check.py --category structure
python3 packages/health_check.py --category config
python3 packages/health_check.py --category governance
```
...
| Core directories | Missing dirs | `mkdir -p packages docs tests` |
| Core files | Missing Python files | Restore from GitHub (Phase 1) |
| Config directory | No packages/.sdd | `mkdir -p packages/.sdd` |
| Python version | Version < 3.8 | Upgrade Python |
| Governance config | Invalid JSON | `python3 packages/governance_compliance.py --fix-steps` |
...
find packages -type f -name "*.py" | head -20

# 2. List missing from Phase 1/2
# Expected files:
#   packages/health_check.py
#   packages/agent_handshake.py
#   packages/quiz_executor.py
#   packages/governance_compliance.py
...
cp -r sdd-backup/packages/* /path/to/project/packages/
...
if [ $? -ne 0 ]; then
    ...
    echo "Run: python3 packages/health_check.py --verbose"
    exit 1
fi
```
...
python3 packages/health_check.py >> logs/health.log
...
0 */6 * * * cd /path/to/project && python3 packages/health_check.py --verbose >> logs/health-monitor.log
...
**A**: Yes, extend in `packages/health_check.py`. Add new checks, categories, or validation rules as needed.
...
- **Governance problems?** See [GOVERNANCE_IMPLEMENTATION.md](../packages/.sdd-wizard/templates/governance/base-seedling/GOVERNANCE_IMPLEMENTATION.md)
