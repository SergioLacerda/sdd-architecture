...
def _quick_governance_check(self):
    ...
    return Path("packages/.sdd/governance-core.json").exists()
```
...
self.core_exists = Path("packages").exists()    # 1 syscall
self.health_exists = Path("packages/health_check.py").exists()  # 1 syscall

# After: Batch queries
files_to_check = [".git", "packages", "packages/health_check.py"]
...
python3 packages/health_check.py --verbose 2>&1 | tee -a logs/perf.log
...
python3 packages/health_check.py --category git --verbose
python3 packages/health_check.py --category structure --verbose
python3 packages/health_check.py --category config --verbose
python3 packages/health_check.py --category governance --verbose

# 2. Profile with timing
python3 -m cProfile -s cumulative packages/health_check.py > profile.txt
...
ls -lh packages/.sdd/governance-core.json
...
json.load(open('packages/.sdd/governance-core.json'))
print(f'{(time.perf_counter()-start)*1000:.1f}ms')
"

# 3. Use quick mode
python3 packages/governance_compliance.py --verify --mode quick
```
