...
import pytest
from sdd_wizard.sdd_runtime.wizard_orchestrator import WizardOrchestrator
...
class TestPhase5Orchestrator:
    ...
    def test_orchestrator_governance_items_count(self, orchestrator):
        ...
        assert summary.get("total_items") == 153
        assert summary.get("core_items") == 2
        assert summary.get("client_items") == 151
        assert summary.get("customizable_items") == 151
        assert summary.get("immutable_items") == 2
