...
class TestPhase1Pipeline:
    ...
    def test_pipeline_generatespackages_and_client_files(self, pipeline, output_dir):
        """Test that pipeline generates both core and client JSON files"""
        result = pipeline.save_outputs(output_dir)

        assert Path(result["governancepackages"]).exists(), "governance-core.json not generated"
        assert Path(result["governance_client"]).exists(), "governance-client.json not generated"

    def testpackages_json_structure(self, pipeline, output_dir):
        """Test governance-core.json has correct structure"""
        result = pipeline.save_outputs(output_dir)

        with open(result["governancepackages"]) as f:
    ...
    def test_client_json_structure(self, pipeline, output_dir):
        ...
        assert "fingerprintpackages_salt" in client_data
        assert len(client_data["fingerprint"]) == 64  # SHA-256 hex

    def testpackages_items_not_customizable(self, pipeline, output_dir):
    ...
    def testpackages_fingerprint_used_as_salt(self, pipeline, output_dir):
        ...
        core_fingerprint_salt = client_data["fingerprintpackages_salt"]
    ...
    def test_fingerprint_consistency(self, pipeline, output_dir):
        ...
        pipeline2 = PipelineBuilder("_spec")
    ...
    def test_at_least_onepackages_item_exists(self, pipeline, output_dir):
