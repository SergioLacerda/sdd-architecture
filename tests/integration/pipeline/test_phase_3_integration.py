...
class TestPhase3Integration:
    ...
    def test_end_to_end_fingerprints_consistent(self):
        ...
        phase1packages_fp = phase1_result["governancepackages"]["fingerprint"]
        ...
        assert phase1packages_fp == phase2_result["core_fingerprint"], "Core fingerprint changed in compiler"
        assert phase1_client_fp == phase2_result["client_fingerprint"], "Client fingerprint changed in compiler"

    def testpackages_fingerprint_salt_strategy(self):
    ...
    def test_compiler_idempotence(self):
        ...
        # Should be identical
        assert size1packages == size2packages, "Compiler not idempotent (size differs)"
    ...
    def test_separation_preserved_through_compilation(self):
        ...
        # Verify separation
        phase1packages_count = len(phase1_result["core_items"])
        phase1_client_count = len(phase1_result["client_items"])
        phase2packages_count = phase2_result["core_item_count"]
        phase2_client_count = phase2_result["client_item_count"]

        assert phase1packages_count == phase2packages_count, "Core item count changed"
